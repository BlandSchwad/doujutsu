...
module.exports.libraryController = async function (req, res) {
  try {
    let go = await getSeries(req.params.library_id);
...
module.exports.seriesController = async function (req, res) {
  try {
    ...
    res.status(200).send({ seriesInfo, bookList });
  } catch (err) {
    res.status(500).send(err);
  }
};
...
// module.exports.addPages = function (book) {
//   const reader = new FileReader(book.file_path);
//   //console.log(book.file_path)
//   unzipit
//     .unzip(reader)
//     .then(({ entries }) => {
//       let pageIndex = 0;
//       Object.entries(entries).forEach(([name, entry], index) => {
//         if (
//           name.endsWith(".jpg") ||
//           name.endsWith(".jpeg") ||
//           name.endsWith(".png") ||
