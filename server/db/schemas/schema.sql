...
CREATE TABLE series (
  ...
  name VARCHAR(500) NOT NULL,
  FILE_PATH VARCHAR(500),
  LIBRARY_ID VARCHAR(50) REFERENCES library (id) ON DELETE CASCADE,
  BOOK_COUNT INT
);
...
CREATE TABLE book (
  ...
  NAME VARCHAR(500) NOT NULL,
  FILE_PATH VARCHAR(500),
  SERIES_ID VARCHAR(50) REFERENCES series (id) ON DELETE CASCADE,
  FILE_SIZE INT,
  PAGE_COUNT INT,
  LIBRARY_ID VARCHAR(250) REFERENCES library (id) ON DELETE CASCADE
...
CREATE TABLE page (
  ...
  NUMBER INT, 
  BOOK_ID VARCHAR(250) REFERENCES book (id) ON DELETE CASCADE,
  FILE_SIZE INT,
...
CREATE view seriesView AS SELECT series.id, series.name, series.file_path, series.library_id, library.name library_name, series.book_count from series JOIN library on series.library_id = library.id; 
